...
cn = sqlite3.connect(r"data\events.db")
cs = cn.cursor()
df = pd.DataFrame()
    ...
    for idx,row in enumerate(df.itertuples()):
        ...
        with st.expander(label=row_header,expanded=False):
            ...
            with c1:
                ...
                if idx in st.session_state.del_ind:
                    bet_cal.append([idx, choice, row.odds_1 if choice == row.home_team else row.odds_y, row.WINNER, "Deleted"])
                else:    
                    bet_cal.append([idx,choice,row.odds_1 if choice == row.home_team else row.odds_y,row.WINNER])
...
cn.commit()
cs.close()
cm = sqlite3.connect(r"data\bets.db")
cr = cm.cursor()
cr.execute("""
...
if st.button("Confirm Parlay"):
    if bet_cal.size>0:
        ...
        VALUES(?,?,?,?)
        """
        cr.execute(sql_query,(money_bet
                              ,row_odds*money_bet
